...
flo_aws = FloAws()
RESOURCE_BUCKET = "flo-public"
METADATA_BUCKET = "flo-metadata"
...
def resources_get_metadata():
    try:
        resp = flo_aws.s3.get_object(Bucket=METADATA_BUCKET, Key="resourceMaps.json")
...
def resources_get_by_id():
    try:
        ...
        resp = flo_aws.s3.list_objects_v2(Bucket=RESOURCE_BUCKET, Prefix="resources/markdowns_current")
        for obj in resp["Contents"]:
            ...
            result.append({
                "resource_url": flo_aws.S3_BUCKET + object_key,
...
Base.metadata.create_all(flo_aws.engine)

# GET
@resource_api.route('/resources/<id>')
def resources_get_by_id(id):
    try: 
        resp = flo_aws.rds.query(Resource).filter(Resource.id == id).first()
...
def resources_put_by_id(id):
    try: 
        ...
        # Get the resource
        resource = flo_aws.rds.query(Resource).filter(Resource.id == id).first()
...
def resources_get_by_category(category):
    try:
        ...
        query = flo_aws.rds.query(Resource).filter(Resource.category == category).order_by(Resource.timestamp.desc())
...
def resources_get_by_author(author):
    try: 
        resp = flo_aws.rds.query(Resource).filter(Resource.author == author).all()
        current_app.logger.info(f"[GET] response: {resp}")
        # resource is not in metadata table
        if len(resp) == 0:
